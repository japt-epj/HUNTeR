...
public class QuizController implements ch.japt.epj.api.QuizApi, PaginatedQuiz {
  ...
  public ResponseEntity<Void> addQuiz(@Valid @RequestBody QuizDto body) {
  ...
  @Override
  @Secured({"ROLE_TEACHER"})
  public ResponseEntity<List<QuizDto>> quizIdGet(@Valid @PathVariable("id") List<Integer> id) {
    return new ResponseEntity<>(quizModel.getQuizzes(id), HttpStatus.OK);
  }

  @Override
  @Secured({"ROLE_TEACHER"})
  public ResponseEntity<Page<QuizDto>> quizGet(
